...
# A pre-2026-09-10 LOSO cache has no calib split (session-wise D5 re-prep added it);
# reusing one silently makes every stage-1 trial exit 1 and prune, "succeeding" with zero
# complete trials. Only LOSO defines calib, so other schemes skip the check.
[ "${FOLD_SCHEME}" != LOSO ] || [ -f "${DATA_CACHE}/folds/LOSO/fold_00_calib.npy" ] || { echo "stale dataset cache: ${DATA_CACHE} has no LOSO calib split (pre-2026-09-10 prep) — re-upload ${DATASET_SRC} from a current prep, delete the cache, and resubmit"; exit 1; }
