...
*Written 2026-09-05. Runs the four stage-1 Optuna studies (one per DepthwiseCNN configuration) and brings the checkpoints back. Every command is one line; run the blocks in order. Where a job id appears as `123456`, substitute the id `sbatch` printed.*
...
**B4. Fold coverage** — must print `15` four times:

```
for t in 2353 3408 3650 4223; do ls $HOME/data/smatable-trial-$t/folds/LOSO | grep -c train.npy; done
```
...
**B6. Three-epoch smoke on the login node** (~1 min). Same configuration as the local anchor run; the local result was `accuracy=0.560000` with `n_params=434`. Mac-vs-x86 bit-identity was verified for the toy example in April but not yet for this trainer, so if the accuracy differs, send me the RESULT line — it is information, not a failure.
