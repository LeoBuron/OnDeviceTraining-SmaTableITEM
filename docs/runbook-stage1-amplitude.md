...
Expected wall clock, from local per-trial timings and whole waves of 48 workers: trial-3650 about 3 h (150 trials = four waves of ~46 min); trial-3408 about 10 h (75 trials = two waves of ~4.9 h: a single-process Release smoke on 2026-09-10 measured 70 s/epoch, the July 17.5 h figure was measured with 4 concurrent local workers), far inside the 96 h partition cap, so its sbatch time and `TRIAL_TIMEOUT_S` are generous margins; trial-2353 and trial-4223 are unmeasured at 250 epochs, 48 h is a guess with margin. Because the fold is a grid dimension, `GridSampler` re-evaluates a few cells at 48 workers; the duplicates are pruned after the fact and are harmless. The wrapper now caches each dataset under its own name in the workspace (fixed 2026-09-05; before, every study silently reused the first dataset). The trial-3650 timing was measured under the old split; the train set is now 7559 instead of 8399 windows, so its epochs run slightly faster (the trial-3408 smoke already ran on the re-prepped data).
